...
//                }
                ...
                String sql="UPDATE food_category SET category=? WHERE id=?";
                PreparedStatement ps=con.prepareStatement(sql);
                ps.setString(1,catnm);
                ps.setInt(2,id);
                int x=ps.executeUpdate();
                
                if(x==1)
                {
                    out.println("<script>alert('Updated Successfully'); location.replace('viewcat.jsp')</script>");
                }
            }
            catch(Exception e)
            {
